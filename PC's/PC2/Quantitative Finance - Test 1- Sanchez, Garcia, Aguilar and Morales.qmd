...
Testing if the dataset is stationarity

```{r}
# Augmented Dickey-Fuller test for stationarity
adf.test(BTC.split$data.xts)
```

Testing if the training set is stationarity

```{r}
# Augmented Dickey-Fuller test for stationarity
adf.test(BTC.split$train.xts)
```
...
```{r}
# Augmented Dickey-Fuller test for stationarity
adf.test(BTC.split$test.xts)
```

# 7. Try and fit several SARIMA GARCH models

Getting the actuals. Will be used for smape
```{r}
actuals <- BTC.split$test.xts
```


Grid-searching the best models
```{r}
ar_search = 0:1; ma_search = c(0:1); search_gar = 0:1; search_arch = 0:1; objective_time_series <- BTC.split$train.xts; forecast_steps = length(BTC.split$test.xts)

models <- expand.grid(gar = search_gar, arch = search_arch, ar =ar_search, ma = ma_search) %>%
  rowwise() %>%
  mutate(
    model_spec = list(ugarchspec(variance.model = list(garchOrder=c(gar,arch)), 
                                 mean.model = list(armaOrder=c(ar,ma)))),
    model_fit = list(ugarchfit(spec = model_spec, data = objective_time_series, solver = "hybrid")),
    model_forecast = list(ugarchforecast(fitORspec = model_fit, n.ahead = forecast_steps)),
    smape = smape(a = actuals, f = model_forecast@forecast$seriesFor[[1]])
  )

```


Plot the forecasts vs the test data of the model with the highest smape
```{r}
# best_model <- models[which.min(models$smape),]
# forecasts_bSMAPE <- best_model$model_forecast[[1]]@forecast$seriesFor
# forecasts_bSMAPE.df <- data.frame(
#   ret = as.numeric(forecasts_bSMAPE),
#   date = index(BTC.split$test.xts)
# )
# # plot the forecasts and the test data
# ggplot() +
#   geom_line(data = BTC.split$test.df, aes(x = date, y = ret), color = "blue") +
#   geom_line(data = forecasts_bSMAPE.df, aes(x = date, y = ret), color = "red") +
#   labs(title = "Forecasts vs Test Data",
#        x = "Date",
#        y = "Returns") +
#   theme_minimal()
```
