...
1.  **create_card_deck**
    -   Input: amount of cards (default value 73)
    -   Output: dataframe with main number and modifier
2.  **deal_cards**
    -   Input: number of players (default value 2)
    -   Output: vector with cards for each player and cards on discard pile
3.  **initialize_game**
    -   Input: cards dataframe, vector from deal_cards
    -   Output: list with Players (dataframe for each player) and Discard Pile (dataframe)
4.  **shuffle_discard_pile**
    -   Input: discard pile dataframe

    -   Output: shuffled discard pile dataframe
5.  **play_card**
...
```{r}
n = 2
games_list <- purrr::map(1:n, ~ initialize_game(num_cards = 73, number_of_players = 1, min_discard_pile = 1) |>
                    shuffle_discard_pile())
