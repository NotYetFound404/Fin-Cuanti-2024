...
execute_transactions <- function(balanced_signals, portfolio.st) {
  for (i in 1:nrow(balanced_signals)) {
    ...
    pair <- gsub("\\.price", "", pair)#eliminar .price
...
dailyEqPL(account.st)
chart.Posn(portfolio.st, Symbol = spread_names[1])
getTxns(Portfolio = portfolio.st, Symbol = spread_names[1])
getTxns(Portfolio = portfolio.st, Symbol = spread_names[2])
