...
# Función para inicializar el portafolio y la cuenta
initialize_portfolio <- function() {
  initPortf(portfolio.st, symbols = spread_names, initDate = from, currency = "USD")
  initAcct(account.st, portfolios = portfolio.st, initDate = from, initEq = initEq, currency = "USD")
  initOrders(portfolio = portfolio.st, initDate = to)
}
...
# Función para actualizar el estado del portafolio
update_portfolio_state <- function(date, daily_signals) {
    ...
    if (nrow(daily_signals) == 0) {
      ...
      newRow <- portfolio_state;newRow$Date <- date
      portfolio_state <<- rbind(portfolio_state, newRow)
  ...
  daily_pnl <- equity - portfolio_state$Equity[nrow(portfolio_state)]
  ...
  portfolio_state <<- rbind(portfolio_state, data.frame(
    ...
    Pct_Invested = (equity - available_cash) / equity,
    Pct_Margin_Used = margin_used / equity,
    Pct_Cash_Available = available_cash / equity
  ))
  return(portfolio_state)
}
...
# 
# 
# #blotter built in functions
# calcPortfWgt(Account = account.st, Portfolio = portfolio.st) #permite calcular el % del portafolio invertido en cada activo
# 
# AcctReturns(Account = account.st) #retornos totales de la cuenta
# PortfReturns(Account = account.st) #retornos de cada pareja
# 
# dailyTxnPL(Portfolios = portfolio.st)
# 
# dailyStats(Portfolios = portfolio.st,perSymbol = TRUE) #obtener estadísticas diarias)
# 
...
# 
# 
# getPosQty(Portfolio = portfolio.st, Symbol = "GOOGL_TSLA", Date = to) #obtener la cantidad de un activo)
# getTxns(Portfolio = portfolio.st, Symbol = "GOOGL_TSLA") #obtener las transacciones de un activo))
# 
# port <- getPortfolio(portfolio.st)
# por.Summ <- port$summary
# por.Symb<- port$symbols
# por.Symb[[1]]
# por.Symb[[2]]
# 
# acc <- getAccount(account.st)
# acc.Summ <- acc$summary
# 
# 
...
# cumsum(port.Ret)
# initEq
# getEndEq(Account = account.st, Date = to)
